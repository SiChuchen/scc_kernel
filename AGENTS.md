...
The kernel code is grouped by subsystem. `boot/` contains the stage-1 assembly that enters protected mode, while `init/` hosts early C entry points. Core services live under `drivers/` (device access), `mm/` (memory management), and `kernel/` (debug helpers). Shared headers are kept in `include/`, and reusable C helpers sit in `libs/`. Linker and debugger scripts are under `scripts/`. The `floppy.img` disk image receives the built binary, and the linked kernel lands in `scc_kernel/`. Place new hardware-specific assembly beside related C code and expose public interfaces through headers in `include/`.

## Build, Test, and Development Commands
Use `make` to compile all `.c` and `.s` sources and produce `scc_kernel`. Run `make update_image` to refresh `floppy.img` (requires `sudo` for loopback mounting). `make qemu` boots the image in QEMU for a quick smoke test, and `make debug` launches QEMU paused with `cgdb` preloaded from `scripts/gdbinit`. Clean intermediate objects with `make clean`. When iterating frequently, prefer `make` followed by `make qemu` to avoid repeated privileged mounts.

## Coding Style & Naming Conventions
Follow the existing kernel style: four-space indentation, no tabs, and K&R-style braces on the next line for functions. Keep identifiers descriptive and lower_snake_case (`console_putc_color`, `real_color_t`). Header guards adopt the `INCLUDE_<NAME>_H_` pattern. Place prototypes in `include/` alongside related `struct` definitions, and keep subsystem-private helpers `static`. Inline comments are concise and explain intent rather than mechanics; prefer English for new comments.

## Testing Guidelines
There is no automated test harness today; manual verification is essential. After code changes, rebuild with `make` and boot with `make qemu`, confirming expected console output or interrupts. For deeper investigation, attach with `make debug` and use the breakpoints defined in `scripts/gdbinit`. When adding features, document expected boot messages and edge checks in your pull request so reviewers can reproduce them.

## Commit & Pull Request Guidelines
Write commit subjects in imperative mood (`Add PIT tick handler`) and keep them under 72 characters. Group related changes together and explain rationale and side effects in the body if the diff is non-trivial. Pull requests should describe the subsystem touched, outline testing (e.g., `make qemu`, `make debug` breakpoint walk-through), and note any required kernel configuration or disk image steps. Include screenshots or serial logs when behavior changes are user-visible.

## Environment & Security Notes
Mounting `floppy.img` modifies files with elevated privileges; verify you have a clean unmounted state before running `make update_image`. Avoid committing generated `.o` files or the binary image, and ensure local `/mnt/kernel` mounts are removed (`make umount_image`) before leaving the workspace.
